  ...
  return (
    <Card className="h-full transition group-hover:border-amber-100 group-hover:shadow-md">
      ...
      <CardContent className="space-y-4">
        {issues.length === 0 ? <p className="text-sm text-muted-foreground">—</p> : issues.map((issue) => (
          <div key={`${issue.documentId}-${issue.title}`} className="flex items-center justify-between rounded-lg border px-3 py-2 transition group-hover:border-amber-200">
            <div>
