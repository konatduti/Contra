  ...
  return (
    <Card className="h-full transition group-hover:border-amber-100 group-hover:shadow-md">
      ...
      <CardContent className="space-y-4">
        {actionDocs.length === 0 ? <p className="text-sm text-muted-foreground">No documents found.</p> : actionDocs.map((doc) => (
          <div key={doc.id} className="flex items-center justify-between rounded-lg border px-4 py-3 transition group-hover:border-amber-200">
            <div>
